Shader "Custom/Research/Multiple Lights" {
	...
	SubShader {
		...
		Pass {
			// Since this is the second pass of lightning
			// We tell it to Add it to the Base pass.
			Tags { "LightMode" = "ForwardAdd" }
			
			// We tell that this pass should blend the source (base) and destination (this)
			// Colors through fully.
			// We could use for exemple Zero (won't use a single color), SrcColor (this color * previous color)
			// You can find more: http://docs.unity3d.com/Manual/SL-Blend.html
			// Blend (This Pass) (Previous Pass)
			// Ex: Blend Zero One (Zero from this, All from previous)
			// Ex: Blend One Zero (All from this, Zero from previous)
			Blend One One
			
			// Now, we tell how this pass should work, since it is really the EXACTLY same work
			// We can just copy paste the above code, with just those difference in Tags and Blend
